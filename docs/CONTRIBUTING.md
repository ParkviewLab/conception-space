...
conception-space follows the ParkviewLab conventions. The essentials:
...
conception-space is plain JavaScript today (no TypeScript or test suite yet), so
...
conception-space is **dual-licensed**: code is **AGPL-3.0-or-later** (a
commercial license is available — see [`../LICENSING.md`](../LICENSING.md)), docs
are **CC-BY-4.0**, and the ParkviewLab logo is proprietary. Every new file needs
an SPDX header or a `REUSE.toml` entry, or `reuse lint` breaks: code/config/CI →
`AGPL-3.0-or-later`, docs → `CC-BY-4.0`.
